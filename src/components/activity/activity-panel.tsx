...
interface ActivityPanelProps {
  ...
  loading?: boolean
  aiDetailsLoading?: boolean
  onSelectDay: (day?: ActivityDaySummary) => void
  onRefresh?: () => void | Promise<void>
  onRequestAiDetails?: (force?: boolean) => void | Promise<void>
...
function AiInteractionTab({
  ...
  return (
    <div className="space-y-4">
      {detailsLoading ? (
        <div className="flex items-center gap-2 rounded-lg border border-border/70 bg-muted/25 px-3 py-2 text-xs text-muted-foreground">
...
export function ActivityPanel({
  ...
  return (
    <div className={cn('flex h-full min-h-0 flex-col', mode === 'page' ? 'gap-5' : 'gap-4')}>
      ...
      <Tabs value={tab} onValueChange={(value) => handleTabChange(value as ActivityTab)} className="flex min-h-0 flex-1 flex-col">
        <div className="flex items-center gap-2">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pr-1">
          ...
          <TabsContent value="ai" className="mt-4">
            <AiInteractionTab data={data} onOpenEntryPath={handleOpenEntryPath} detailsLoading={aiDetailsLoading} />
          </TabsContent>
