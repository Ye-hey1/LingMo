...
async function markSkillRuntimeChanged() {
  try {
    const { markSkillsChangedForAgent } = await import('@/lib/skills/agent-ready')
    await markSkillsChangedForAgent()
  } catch (error) {
    console.warn('[Skills] Failed to notify runtime skill change:', error)
  }
}
...
interface SkillsState {
  ...
  deleteInstalledSkill: (id: string) => Promise<void>
  setInstalledSkillEnabled: (id: string, enabled: boolean) => Promise<void>
...
`
  ...
  fetchSkills: async () => {
    set({ loading: true })
    try {
      const installedSkills = await invoke<SkillRecord[]>('skill_v2_get_all')
      await get().refreshSkills()
      set({ installedSkills })
    } finally {
      set({ loading: false })
  ...
  setInstalledSkillEnabled: async (id, enabled) => {
    await invoke('skill_v2_set_enabled', { id, enabled })
    await get().refreshSkills()
    const installedSkills = get().installedSkills.map(s => s.id === id ? { ...s, enabled } : s)
    set({ installedSkills })
  },
  ...
  importDiscovered: async (id) => {
    ...
    await get().refreshSkills()
    set({
      installedSkills: [record, ...get().installedSkills],
  ...
  installFromGit: async (url, name) => {
    ...
    try {
      ...
      await get().refreshSkills()
      set({ installedSkills: [record, ...get().installedSkills] })
      return record
    } finally {
  ...
  fetchLeaderboard: async (board) => {
    ...
    try {
      const marketSkills = await invoke<MarketSkill[]>('skill_v2_fetch_leaderboard', { board })
      set({ marketSkills })
      return marketSkills
    } finally {
  ...
  searchMarket: async (query, limit) => {
    ...
    try {
      const marketSkills = await invoke<MarketSkill[]>('skill_v2_search_skillssh', {
        query,
        limit: limit ?? 60,
      })
      set({ marketSkills })
