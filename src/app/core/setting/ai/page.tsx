...
export default function AiPage() {
  ...
  // 当前选中的AI配置
  const currentConfig = allModelConfigs.find(model => model.key === selectedAiConfig)
  ...
  const deleteCurrentConfig = async () => {
    ...
    if (updatedList.length > 0) {
      setSelectedAiConfig(updatedList[0].key)
    } else {
      setSelectedAiConfig('')
    }
  }
  ...
  useEffect(() => {
    async function init() {
      ...
      // 所有模型配置
      const allModels = aiModelList || []

      // 如果已经有保存的选择，且该配置仍然存在，则使用它
      if (selectedAiConfig && allModels.find(model => model.key === selectedAiConfig)) {
        // 已经有保存的选择，不需要做任何事情
        return
      } else if (allModels.length > 0) {
        // 如果没有保存的选择或选择的配置不存在，选择第一个
        const firstUserModel = allModels[0]
  ...
  return (
    <SettingType id="ai" icon={<BotMessageSquare />} title={t('title')} desc={t('desc')}>
      {/* 当没有用户自定义模型时显示默认模型区域 */}
      {allModelConfigs.length === 0 && <DefaultModelsSection />}
      
      <CreateConfig 
        hasCustomModels={allModelConfigs.length > 0} 
