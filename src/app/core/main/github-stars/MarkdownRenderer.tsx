...
import { safeWriteText, getClipboardErrorMessage } from '@/lib/github-stars/clipboardUtils';
...
const CodeBlock: React.FC<{
  ...
  return (
    <div className={`relative group my-3 rounded-xl overflow-hidden border shadow-md ${
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-muted/30 border-b border-black/[0.06] dark:border-white/[0.04]">
              ...
              isBashLike
                ...
                : isPowerShell
                  ...
                  : isCmdLike
                    ...
                    : 'bg-muted text-muted-foreground border border-black/[0.06] dark:border-white/[0.04]'
            }`}>
              {isBashLike && (
      ...
      {copyError && (
        <div className="absolute top-14 right-4 max-w-xs bg-popover text-popover-foreground text-xs px-3 py-2 rounded-lg shadow-lg z-20 border border-border">
          {copyError}
        </div>
      )}
...
const MarkdownImage: React.FC<{ src?: string; alt?: string; baseUrl?: string }> = ({
  ...
  if (hasError) {
    return (
      <div className="my-2 px-3 py-2 bg-muted rounded border border-border flex items-center gap-2 text-xs">
        ...
        {alt && <span className="text-muted-foreground/80 truncate max-w-[120px]">{alt}</span>}
        <button
          onClick={handleRetry}
          className="ml-auto px-2 py-0.5 text-xs text-primary hover:opacity-85 transition-colors flex-shrink-0"
        >
          重试
        </button>
      </div>
    ...
    <>
      {isSmallImage ? (
        <span className="inline-flex items-center my-1">
          {isLoading && (
            <span className="w-20 h-7 bg-muted rounded animate-pulse inline-block" />
          )}
          <span className="relative inline-block">
            <NextImage
              ...
              src={imageUrl}
              alt={alt || ''}
              width={naturalWidth || 300}
              height={naturalHeight || 120}
              className={`
                h-auto rounded
                ${isInsideLink
                  ? 'hover:opacity-80'
        ...
        <div className="my-4 flex flex-col items-center group/img">
          {isLoading && (
            <div className="w-full max-w-md h-16 bg-muted rounded-lg flex items-center justify-center animate-pulse gap-2">
          ...
          <div className={`relative inline-block rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300 ${isLoading ? 'hidden' : ''}`}>
            <NextImage
              ...
              src={imageUrl}
              alt={alt || ''}
              width={naturalWidth || 960}
              height={naturalHeight || 540}
              className={`
                h-auto rounded-xl
                ${isInsideLink
          ...
          {!isLoading && !hasError && (
            <div data-translate="false" className="text-center mt-2 text-xs text-muted-foreground opacity-0 group-hover/img:opacity-100 transition-opacity duration-200 flex items-center gap-3">
              <span>
                {isInsideLink
                  ? '单击放大 · Ctrl+点击打开链接'
                  : '点击可放大'
          ...
          {!isLoading && !hasError && isInsideLink && parentLinkHref && (
            <div
              ...
              className="text-center mt-1 text-xs text-primary opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center gap-1 cursor-pointer"
              onClick={(e) => {
                e.stopPropagation();
                e.preventDefault();
                window.open(parentLinkHref, '_blank', 'noopener,noreferrer');
...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = memo(({
  ...
  const markdownComponents: Components = useMemo(() => ({
    a: (props: any) => <MarkdownLink {...props} baseUrl={baseUrl} headingIds={headingIds} />,
    img: (props: any) => <MarkdownImage {...props} baseUrl={baseUrl} />,
    h1: ({ children }: any) => {
      const id = getHeadingId(children);
      return <h1 id={id} className="text-lg font-bold text-foreground mt-4 mb-2">{children}</h1>;
    },
    h2: ({ children }: any) => {
      const id = getHeadingId(children);
      return <h2 id={id} className="text-base font-semibold text-foreground mt-3 mb-2">{children}</h2>;
    },
    h3: ({ children }: any) => {
      const id = getHeadingId(children);
      return <h3 id={id} className="text-sm font-medium text-foreground mt-2 mb-1">{children}</h3>;
    },
    h4: ({ children }: any) => {
      const id = getHeadingId(children);
      return <h4 id={id} className="text-sm font-medium text-foreground mt-2 mb-1">{children}</h4>;
    },
    h5: ({ children }: any) => {
      const id = getHeadingId(children);
      return <h5 id={id} className="text-sm font-medium text-muted-foreground mt-1 mb-1">{children}</h5>;
    },
    h6: ({ children }: any) => {
      const id = getHeadingId(children);
    ...
    code: ({ className, children, ...props }: any) => {
      ...
      return isInline ? (
        <code className="px-1.5 py-0.5 bg-muted text-foreground rounded text-xs font-mono" {...props}>
          {children}
        </code>
      ) : (
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="border-l-4 border-border pl-4 py-1 my-2 text-muted-foreground italic bg-muted/40 rounded-r">
        {children}
      </blockquote>
    ),
    ...
    tbody: ({ children }: any) => <tbody className="text-foreground">{children}</tbody>,
    th: ({ children }: any) => (
      ...
      if (props.type === 'checkbox') {
        return (
          <input
            {...props}
            readOnly
            className="mr-1.5 align-middle w-3.5 h-3.5 rounded border-input text-primary focus:ring-primary dark:border-white/[0.08]"
          />
        );
      }
      return <input {...props} />;
