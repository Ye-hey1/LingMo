...
import {
  ...
  unwrapSpans,
} from '@/lib/github-stars/domTextScanner';
import { translateBatch, type TranslateResult } from '@/lib/github-stars/translate';
import { detectLanguage, getTranslateDirection, cleanTranslatedText } from '@/lib/github-stars/markdownSplitter';
...
.bimd-mode-translated [data-bi-original] { display: none !important; }
.bimd-mode-original [data-bi-translation] { display: none !important; }
[data-bi-translation] code {
...
const BilingualMarkdownRenderer = forwardRef<BilingualMarkdownRendererHandle, BilingualMarkdownRendererProps>(({
  ...
  const translate = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        wrapper.className =
          'mt-1 pl-3 border-l-2 border-primary text-muted-foreground text-sm leading-relaxed';
  ...
  return (
    <div className={`bilingual-markdown ${modeClass}`}>
      ...
      {!isControlled && (
        <div className="flex items-center justify-end gap-1 mb-3 pb-2 border-b border-border">
          {[
            {
