...
import { Network, Keyboard, Feather, Link2, GitBranch, X } from 'lucide-react'
    ...
    return (
      <div className="h-7 flex items-center justify-between gap-3 px-3 border-t border-border bg-background text-xs text-muted-foreground">
        ...
        <div className="shrink-0 flex items-center gap-1">
          <IconButton title="反向链接" onClick={() => setBacklinksOpen(true)}>
        ...
        <Dialog open={backlinksOpen} onOpenChange={setBacklinksOpen}>
          <DialogContent className="max-h-[70vh] overflow-y-auto p-0 sm:max-w-[400px]">
            <BacklinksDialogContent onClose={() => setBacklinksOpen(false)} />
          </DialogContent>
        </Dialog>
      </div>
    )
  ...
  return (
    <div className="h-6 flex items-center justify-between px-2 border-t border-border bg-background text-[11px] text-muted-foreground select-none gap-0.5">
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        ...
        <IconButton title="相关笔记" onClick={() => setRelatedOpen(true)}>
      ...
      {/* Backlinks Dialog */}
      <Dialog open={backlinksOpen} onOpenChange={setBacklinksOpen}>
        <DialogContent className="max-h-[70vh] overflow-y-auto p-0 sm:max-w-[420px] rounded-lg">
          <BacklinksDialogContent onClose={() => setBacklinksOpen(false)} />
        </DialogContent>
      </Dialog>

      {/* Related Notes Dialog */}
      <Dialog open={relatedOpen} onOpenChange={setRelatedOpen}>
        <DialogContent className="max-h-[70vh] overflow-y-auto p-0 sm:max-w-[420px] rounded-lg">
          <RelatedNotesDialogContent onClose={() => setRelatedOpen(false)} />
        </DialogContent>
      </Dialog>
    </div>
  )
...
function BacklinksDialogContent({ onClose }: { onClose: () => void }) {
  const activeFilePath = useArticleStore((s) => s.activeFilePath)
    ...
    <div>
      <DialogHeader className="flex flex-row items-center justify-between border-b px-4 py-2.5 space-y-0">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs text-amber-600 hover:bg-amber-50 dark:text-amber-400 dark:hover:bg-amber-950/40 transition-colors"
          >
      ...
      <div className="p-2">
        ...
        {aiError && aiState !== 'preview' && aiState !== 'done' && (
          <div className="px-2 py-2 text-xs text-muted-foreground">{aiError}</div>
        )}
        {aiState === 'preview' && aiSuggestions.length > 0 && (
          <div className="border rounded-md mb-2 p-2">
            <div className="px-1 py-1 text-[11px] text-muted-foreground">发现 {aiSuggestions.length} 个可创建的双链：</div>
            {aiSuggestions.map((s, i) => (
              <div key={`ai-${s.target}-${i}`} className="flex items-start gap-2 px-1 py-1 text-xs hover:bg-accent/50 rounded">
        ...
        {aiState === 'done' && (
          <div className="flex items-center gap-2 px-2 py-1.5 text-xs text-green-600 dark:text-green-400 mb-2">
        ...
        {backlinks.length > 0 && (
          <div className="mb-1">
            <div className="px-2 pt-1 pb-1 text-[11px] font-medium text-muted-foreground">入链 ({backlinks.length})</div>
            {backlinks.map((bl, i) => (
              <button key={`bl-${i}`} className="flex w-full items-start gap-2 px-2 py-1.5 text-left text-xs hover:bg-accent/50 rounded transition-colors" onClick={() => { setActiveFilePath(bl.sourcePath); onClose() }}>
        ...
        {outgoingLinks.length > 0 && (
          <div>
            <div className="px-2 pt-1 pb-1 text-[11px] font-medium text-muted-foreground">出链 ({outgoingLinks.length})</div>
            {outgoingLinks.map((target, i) => (
              <button key={`ol-${i}`} className="flex w-full items-start gap-2 px-2 py-1.5 text-left text-xs hover:bg-accent/50 rounded transition-colors" onClick={() => {
        ...
        {backlinks.length === 0 && outgoingLinks.length === 0 && aiState === 'idle' && (
          <div className="px-2 py-6 text-xs text-muted-foreground text-center">暂无链接关系</div>
        )}
      </div>
    </div>
  )
...
function RelatedNotesDialogContent({ onClose }: { onClose: () => void }) {
  const activeFilePath = useArticleStore((s) => s.activeFilePath)
  const setActiveFilePath = useArticleStore((s) => s.setActiveFilePath)
  ...
  return (
    <div>
      <DialogHeader className="border-b px-4 py-2.5 space-y-0">
        <DialogTitle className="text-sm font-medium flex items-center gap-1.5">
      ...
      <div className="p-2">
        {isLoading ? (
          ...
          <div className="px-2 py-8 text-xs text-muted-foreground text-center">暂无相关笔记</div>
        ) : (
          notes.map((note, i) => (
            <button
              key={`rn-${i}`}
              className="flex w-full items-start gap-2 px-2 py-1.5 text-left text-xs hover:bg-accent/50 rounded transition-colors"
