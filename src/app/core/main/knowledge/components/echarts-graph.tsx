...
export function EChartsGraph({ width, height, layoutMode = 'force' }: EChartsGraphProps) {
  ...
  const getOption = useCallback(() => {
    ...
    return {
      ...
      animationDelay: (index: number) => topicMode && !reducedMotion ? Math.min(700, index * 9) : 0,
      animationDelayUpdate: (index: number) => topicMode && !reducedMotion ? Math.min(200, index * 4) : 0,
