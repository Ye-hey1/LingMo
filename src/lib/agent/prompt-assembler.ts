...
function buildSpecializedToolCatalog() {
  return section(
    ...
    [
      ...
      '- **统一知识对象 / 知识库概览 / 跨 notes、marks、AI 热点、记忆、agent runs 检索** → `search_knowledge_objects`, `get_knowledge_object_overview`. Use this as the first stop before broad `safe_grep` or ad hoc file scans.',
      '- **知识库陈旧 / 同步后重建 / 文件整理后刷新 / 索引不同步** → `reindex_knowledge_objects` before retrying knowledge search.',
      '- **当前笔记上下文 / 相关笔记 / backlinks / 语义关系 / 链接建议** → `get_current_note_context` first, then `get_connected_notes`, `get_note_backlinks`, `suggest_links_for_note` when deeper graph detail is needed.',
