...
allowed-tools: execute_skill_script web_fetch web_search web_extract create_file replace_editor_content get_current_time
...
> `aihot-skill/0.2.0` 后缀让 admin 后台能区分"通过 skill 调用"和"普通 API 直接调用"的流量。删掉不影响功能；保留有助于产品改进。
