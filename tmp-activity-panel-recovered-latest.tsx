...
import { Activity, BarChart3, Brain, CalendarDays, Clock3, MessageSquare, RefreshCw, Sparkles } from 'lucide-react'
...
import { TimelineReviewPanel } from '@/components/activity/timeline-review-panel'
...
type ActivityTab = 'overview' | 'timeline' | 'latest' | 'ai' | 'memory'
...
function LatestTab({
  ...
  return (
    <SectionShell title="最新内容" className="flex min-h-[420px] flex-col overflow-hidden">
...
function TimelineTab({
  ...
  const currentRange = getTimelineRange(data, rangePreset, startDay, endDay)
  const normalizedStart = currentRange.start <= currentRange.end ? currentRange.start : currentRange.end
  const normalizedEnd = currentRange.start <= currentRange.end ? currentRange.end : currentRange.start
  const rangeLabel = normalizedStart === normalizedEnd ? normalizedStart : `${normalizedStart} ~ ${normalizedEnd}`
    ...
    <div className={cn('flex h-full min-h-0 flex-col', mode === 'page' ? 'gap-5' : 'gap-4')}>
      ...
      <Tabs value={tab} onValueChange={(value) => setTab(value as ActivityTab)} className="flex min-h-0 flex-1 flex-col">
        <div className="flex items-center gap-2">
          ...
          <TabsTrigger value="latest" className="gap-1.5"><Clock3 className="h-3.5 w-3.5" />Latest</TabsTrigger>
          <TabsTrigger value="ai" className="gap-1.5"><MessageSquare className="h-3.5 w-3.5" />AI</TabsTrigger>
