...
import { Activity, BarChart3, Brain, CalendarDays, HeartPulse, MessageSquare, RefreshCw, Sparkles } from 'lucide-react'
...
type ActivityTab = 'overview' | 'timeline' | 'ai' | 'memory' | 'health'
...
function ErrorList({ data }: { data: ActivityCalendarData }) {
  const errors = data.insights.aiUsage.errors

  if (!errors.length) {
    return <p className="py-6 text-center text-sm text-muted-foreground">今日暂无 AI 请求错误</p>
  }

  return (
    <div className="space-y-2">
      {errors.map((error) => (
        <div key={error.errorKind} className="flex items-center justify-between rounded-md bg-muted/35 px-3 py-2">
          <span className="text-sm">{error.errorKind}</span>
  ...
  return (
    <div className="flex h-full w-full items-center justify-center gap-6">
      <div className="relative h-[126px] w-[126px] shrink-0 rounded-full shadow-inner" style={{ background: `conic-gradient(${segments})` }}>
        <div className="absolute inset-[18px] flex flex-col items-center justify-center rounded-full bg-background shadow-sm">
          <span className="text-2xl font-semibold leading-none tabular-nums">{total}</span>
          <span className="mt-1 text-[11px] text-muted-foreground">{rangeLabel}</span>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <SectionShell title="活动组成" className="flex h-[220px] flex-col overflow-hidden">
          <div className="flex min-h-0 flex-1 items-center">
            <OverviewDonutChart items={sourceBreakdown} rangeLabel={scopeLabel} />
          </div>
        </SectionShell>
...
function TimelineTab({
  ...
  const timelineDays = useMemo(() => {
    const { start, end } = getTimelineRange(data, rangePreset, startDay, endDay)
  ...
  return (
    <div className={cn('flex h-full min-h-0 flex-col', mode === 'page' ? 'gap-5' : 'gap-4')}>
      ...
      <Tabs value={tab} onValueChange={(value) => setTab(value as ActivityTab)} className="flex min-h-0 flex-1 flex-col">
        <TabsList className="h-10 w-full justify-start overflow-x-auto rounded-lg bg-muted/70 p-1">
          ...
          <TabsTrigger value="ai" className="gap-1.5"><MessageSquare className="h-3.5 w-3.5" />AI 交互</TabsTrigger>
          <TabsTrigger value="memory" className="gap-1.5"><Brain className="h-3.5 w-3.5" />记忆</TabsTrigger>
          <TabsTrigger value="health" className="gap-1.5"><HeartPulse className="h-3.5 w-3.5" />健康</TabsTrigger>
        </TabsList>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pr-1">
          ...
          <TabsContent value="health" className="mt-4">
            <div className="grid gap-3 md:grid-cols-3">
              <MetricTile label="AI 成功率" value={`${data.insights.aiUsage.successRate}%`} hint={`${data.insights.aiUsage.failureCount} 次失败`} tone={data.insights.aiUsage.failureCount ? 'rose' : 'green'} />
              <MetricTile label="平均耗时" value={`${data.insights.aiUsage.avgLatencyMs}ms`} hint="LingMo 今日请求" tone="blue" />
              <MetricTile label="活动日志" value={data.totals.totalCount} hint="已写入事件" />
            </div>
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              <SectionShell title="错误类型">
                <ErrorList data={data} />
              </SectionShell>
              <SectionShell title="数据源">
                <div className="grid grid-cols-2 gap-3">
                  <MetricTile label="外部会话" value={data.insights.platforms.reduce((sum, platform) => sum + platform.sessionCount, 0)} hint="可读取会话" />
                  <MetricTile label="记忆访问" value={data.insights.memory.totalAccessCount} hint="累计命中" />
                  <MetricTile label="AI 事件" value={data.insights.aiUsage.todayCount} hint="今日新增" />
                  <MetricTile label="待整理记忆" value={data.insights.memory.staleCount} hint="60 天未访问" />
                </div>
              </SectionShell>
            </div>
          </TabsContent>
